...
export function AssessmentBuilder({ open, assessment, onClose, onSave }: Props) {
  ...
  async function handleSave() {
    // Pegamos os IDs atuais da seleção garantindo que eles existam no banco disponível
    // (para evitar IDs de questões apagadas ou de outros formatos).
    const validIds = [...selectedIds].filter(id => availableQuestions.some(q => q.id === id))
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex items-center justify-between">
                    <p className={`text-sm ${selectedIds.size < questionCount ? "text-amber-600 font-medium" : "text-muted-foreground"}`}>
                      ...
                      {selectedIds.size < questionCount && <span className="ml-2">— Faltam {questionCount - selectedIds.size}</span>}
                    </p>
